...
import {
  ...
  DocumentType,
  DocumentVersion
} from '@features/documents/domain/entities/document.entity';
import { DOCUMENT_REPOSITORY } from '@features/documents/infrastructure/tokens';
import {
  Organization,
  OrganizationMine
...
import { UserProfile } from '@features/users/domain/entities/user.entity';
...
export class DocumentsPageComponent implements OnInit {
  ...
  private readonly docTypesById = signal<Record<number, DocumentType>>({});
  private readonly orgsById = signal<Record<number, string>>({});
  private readonly ownersById = signal<Record<number, UserProfile>>({});
  ...
  readonly modalForceState = signal<Document | null>(null);
  ...
  readonly versionsModalDoc = signal<Document | null>(null);
  ...
  private readonly docRepo = inject(DOCUMENT_REPOSITORY);
  ...
  private loadDocumentTypes(): void {
    this.docRepo.listDocumentTypes().subscribe({
      next: types => {
  ...
  private loadOrganizations(): void {
    ...
    const obs = useGlobal
      ? this.orgRepo.listAll()
      : this.orgRepo.listMine();

    obs.subscribe({
      next: (rows: Organization[] | OrganizationMine[]) => {
   ...
   */
  private loadOwnersFor(docs: Document[]): void {
    ...
    this.userRepo.getProfilesBatch(ids).subscribe({
      next: profiles => {
  ...
  private compareForSort(a: Document, b: Document): number {
  ...
  submitCreate(): void {
    ...
    this.loading.set(true);
    this.docRepo.create(payload, this.form.file!).subscribe({
      next: () => {
        this.loading.set(false);
        this.closeCreate();
        this.reload();
  ...
  private executeDelete(d: Document): void {
    this.docRepo.delete(d.id).subscribe({
      next: () => {
        this.reload();
  ...
  // Forzar estado
  // ---------------------------------------------------------------------------
  openForceState(d: Document): void {
  ...
  submitForceState(): void {
    ...
    this.loading.set(true);
    this.docRepo.forceState(d.id, state).subscribe({
      next: () => {
        this.loading.set(false);
  ...
  // ---------------------------------------------------------------------------
  versionsSummary(d: Document): string {
  ...
  sortedVersions(d: Document): DocumentVersion[] {
    return [...(d.versions ?? [])].sort(
      (a, b) => (b.versionNumber ?? 0) - (a.versionNumber ?? 0)
    );
  }

  /** Nombre del archivo de una version (extraido de fileUrl). */
  versionFileLabel(v: DocumentVersion): string {
    const url = v.fileUrl?.trim();
    if (!url) return 'Archivo sin nombre';
    const withoutQuery = url.split('?')[0];
    const lastSegment = withoutQuery.split('/').filter(Boolean).pop();
    if (!lastSegment) return 'Archivo sin nombre';
    try {
  ...
  versionUploaderLabel(v: DocumentVersion): string {
  ...
  downloadVersion(v: DocumentVersion): void {
    ...
    this.docRepo.downloadVersion(d.id, v.id).subscribe({
      next: response => this.openBlobInNewTab(response, d),
      error: e => this.showApiError(e)
    });
  }

  confirmDeleteVersion(v: DocumentVersion): void {
    const d = this.versionsModalDoc();
    if (!d || v.id == null) return;
    this.openConfirm({
  ...
  private executeDeleteVersion(d: Document, v: DocumentVersion): void {
    if (v.id == null) return;
    this.docRepo.deleteVersion(d.id, v.id).subscribe({
      next: updated => {
   ...
   */
  versionLimitReached(d: Document | null): boolean {
    if (!d) return false;
  ...
  submitNewVersion(): void {
    ...
    this.docRepo.uploadNewVersion(d.id, file).subscribe({
      next: updated => {
