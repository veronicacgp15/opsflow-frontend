...
import { DocumentType, DocumentTypeUpsert } from '@features/documents/domain/entities/document.entity';
...
export class DocumentTypesAdminPageComponent implements OnInit {
  ...
  reload(): void {
    ...
    this.docRepo.listAllDocumentTypesAdmin().subscribe({
      next: rows => {
        this.types.set(rows ?? []);
  ...
  submitCreate(): void {
    ...
    this.loading.set(true);
    const body: DocumentTypeUpsert = {
      name,
      description: this.trimDescription()
    };
    this.docRepo.createDocumentType(body).subscribe({
      next: created => {
        this.loading.set(false);
  ...
  submitEdit(): void {
    ...
    this.docRepo.updateDocumentType(id, body).subscribe({
      next: updated => {
        this.loading.set(false);
  ...
  isRowActive(t: DocumentType): boolean {
    return t.active !== false;
  }

  onToggleActive(t: DocumentType, ev: Event): void {
    ...
    this.docRepo.patchDocumentTypeActive(t.id, wantActive).subscribe({
      next: updated => {
  ...
  openDeleteConfirm(t: DocumentType): void {
    this.confirmReq.set({
  ...
  private runDelete(t: DocumentType): void {
    this.loading.set(true);
    this.docRepo.deleteDocumentType(t.id).subscribe({
      next: dto => {
        this.loading.set(false);
