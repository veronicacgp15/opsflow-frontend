...
import { Role } from '@features/access-control/domain/entities/role.entity';
...
export class UsersAdminPageComponent implements OnInit {
  ...
  readonly roleCatalog = signal<Role[]>([]);
  /**
  ...
  readonly rolesModalUser = signal<User | null>(null);
  ...
  ngOnInit(): void {
    this.reload();
    this.roleRepo.list().subscribe({
      next: r => this.roleCatalog.set(r),
      error: () => this.roleCatalog.set([])
    });
    this.orgRepo.listAll().subscribe({
  ...
  // ---------------------------------------------------------------------------
  toggleUserStatus(event: Event, u: User): void {
  ...
  private requestDeactivate(u: User): void {
    this.openConfirm({
  ...
  private requestActivate(u: User): void {
    this.openConfirm({
  ...
  private executeDeactivate(u: User): void {
    this.userRepo.deactivate(u.id).subscribe({
      next: () => {
        this.reload();
  ...
  private executeActivate(u: User): void {
    this.userRepo.activate(u.id).subscribe({
      next: () => {
        this.reload();
  ...
  private executeRevoke(u: User): void {
    this.userRepo.revokeSession(u.id).subscribe({
      next: () => {
  ...
  saveEdit(): void {
    ...
    this.loading.set(true);
    this.userRepo.update(u.id, body).subscribe({
      next: () => {
        this.loading.set(false);
        this.closeEdit();
        this.reload();
  ...
  saveRoles(): void {
    ...
    this.loading.set(true);
    this.roleRepo.setUserRoles(u.id, names).subscribe({
      next: () => {
        this.loading.set(false);
