...
import { Permission, Role } from '@features/access-control/domain/entities/role.entity';
...
export class RolesAdminPageComponent implements OnInit {
  ...
  readonly permissionsModalRole = signal<Role | null>(null);
  ...
  private loadPermissionsCatalog(): void {
    this.roleRepo.listPermissions().subscribe({
  ...
  submitCreate(): void {
    ...
    this.roleRepo.create(name).subscribe({
      next: () => {
  ...
  submitEdit(): void {
    ...
    this.loading.set(true);
    this.roleRepo.update(id, name).subscribe({
      next: () => {
        this.loading.set(false);
        this.closeModals();
        this.reload();
  ...
  private executeDelete(r: Role): void {
    this.roleRepo.delete(r.id).subscribe({
      next: () => {
        this.reload();
  ...
  openPermissions(r: Role): void {
    ...
    this.roleRepo.getRolePermissions(r.id).subscribe({
      next: ids => {
  ...
  savePermissions(): void {
    ...
    this.roleRepo.setRolePermissions(role.id, ids).subscribe({
      next: () => {
